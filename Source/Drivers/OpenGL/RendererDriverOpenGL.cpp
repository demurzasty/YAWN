...
OID RendererDriverOpenGL::CreateViewport(int width, int height, bool directToScreen) {
    OID id = Base::CreateViewport(width, height, directToScreen);
...
void RendererDriverOpenGL::DestroyViewport(OID id) {
...
void RendererDriverOpenGL::SetViewportSize(OID id, int width, int height) {
...
OID RendererDriverOpenGL::GetViewportColorTexture(OID id) const {
...
OID RendererDriverOpenGL::CreateMaterial() {
    OID id = Base::CreateMaterial();
...
void RendererDriverOpenGL::DestroyMaterial(OID id) {
    mMaterials[*id] = GPUMaterialData();
...
void RendererDriverOpenGL::SetMaterialBaseColor(OID id, const Color4& color) {
    mMaterials[*id].BaseColor = color;

    YAWN_GL_CHECK(glNamedBufferSubData(mMaterialBufferId, *id * sizeof(GPUMaterialData), sizeof(GPUMaterialData), &mMaterials[*id]));
}

void RendererDriverOpenGL::SetMaterialRoughness(OID id, float roughness) {
    mMaterials[*id].Roughness = roughness;

    YAWN_GL_CHECK(glNamedBufferSubData(mMaterialBufferId, *id * sizeof(GPUMaterialData), sizeof(GPUMaterialData), &mMaterials[*id]));
}

void RendererDriverOpenGL::SetMaterialMetallic(OID id, float metallic) {
    mMaterials[*id].Metallic = metallic;

    YAWN_GL_CHECK(glNamedBufferSubData(mMaterialBufferId, *id * sizeof(GPUMaterialData), sizeof(GPUMaterialData), &mMaterials[*id]));
}

void RendererDriverOpenGL::SetMaterialOcclusionStrength(OID id, float strength) {
    mMaterials[*id].OcclusionStrength = strength;

    YAWN_GL_CHECK(glNamedBufferSubData(mMaterialBufferId, *id * sizeof(GPUMaterialData), sizeof(GPUMaterialData), &mMaterials[*id]));
}

void RendererDriverOpenGL::SetMaterialAlbedoTexture(OID id, OID textureId) {
    mMaterials[*id].AlbedoTextureId = IsTextureValid(textureId) ? textureId : Pool::None;

    YAWN_GL_CHECK(glNamedBufferSubData(mMaterialBufferId, *id * sizeof(GPUMaterialData), sizeof(GPUMaterialData), &mMaterials[*id]));
}

void RendererDriverOpenGL::SetMaterialNormalTexture(OID id, OID textureId) {
    mMaterials[*id].NormalTextureId = IsTextureValid(textureId) ? textureId : Pool::None;

    YAWN_GL_CHECK(glNamedBufferSubData(mMaterialBufferId, *id * sizeof(GPUMaterialData), sizeof(GPUMaterialData), &mMaterials[*id]));
}

void RendererDriverOpenGL::SetMaterialMetallicRoughnessTexture(OID id, OID textureId) {
    mMaterials[*id].MetallicRoughnessTextureId = IsTextureValid(textureId) ? textureId : Pool::None;

    YAWN_GL_CHECK(glNamedBufferSubData(mMaterialBufferId, *id * sizeof(GPUMaterialData), sizeof(GPUMaterialData), &mMaterials[*id]));
}

void RendererDriverOpenGL::SetMaterialEmissiveTexture(OID id, OID textureId) {
    mMaterials[*id].EmissiveTextureId = IsTextureValid(textureId) ? textureId : Pool::None;

    YAWN_GL_CHECK(glNamedBufferSubData(mMaterialBufferId, *id * sizeof(GPUMaterialData), sizeof(GPUMaterialData), &mMaterials[*id]));
}

void RendererDriverOpenGL::SetMaterialOcclusionTexture(OID id, OID textureId) {
    mMaterials[*id].OcclusionTextureId = IsTextureValid(textureId) ? textureId : Pool::None;

    YAWN_GL_CHECK(glNamedBufferSubData(mMaterialBufferId, *id * sizeof(GPUMaterialData), sizeof(GPUMaterialData), &mMaterials[*id]));
}

OID RendererDriverOpenGL::CreateMesh(int vertexCount, int indexCount) {
    OID id = Base::CreateMesh(vertexCount, indexCount);
...
void RendererDriverOpenGL::DestroyMesh(OID id) {
...
void RendererDriverOpenGL::SetMeshData(OID id, const ArrayView<const Vertex3D>& vertices, const ArrayView<const int>& indices) {
...
OID RendererDriverOpenGL::CreateInstance() {
    OID id = Base::CreateInstance();
...
void RendererDriverOpenGL::DestroyInstance(OID id) {
    mInstances[*id] = GPUInstanceData();
...
void RendererDriverOpenGL::SetInstanceTransform(OID id, const Matrix4& transform) {
    mInstances[*id].Transform = transform;

    YAWN_GL_CHECK(glNamedBufferSubData(mInstanceBufferId, *id * sizeof(GPUInstanceData), sizeof(GPUInstanceData), &mInstances[*id]));
}

void RendererDriverOpenGL::SetInstanceMaterial(OID id, OID materialId) {
    mInstances[*id].MaterialId = IsMaterialValid(materialId) ? materialId : Pool::None;

    YAWN_GL_CHECK(glNamedBufferSubData(mInstanceBufferId, *id * sizeof(GPUInstanceData), sizeof(GPUInstanceData), &mInstances[*id]));
}

void RendererDriverOpenGL::SetInstanceMesh(OID id, OID meshId) {
    mInstances[*id].MeshId = IsMeshValid(meshId) ? meshId : Pool::None;

    YAWN_GL_CHECK(glNamedBufferSubData(mInstanceBufferId, *id * sizeof(GPUInstanceData), sizeof(GPUInstanceData), &mInstances[*id]));
}

void RendererDriverOpenGL::SetInstanceViewport(OID id, OID viewportId) {
    mInstances[*id].ViewportId = IsViewportValid(viewportId) ? viewportId : Pool::None;

    YAWN_GL_CHECK(glNamedBufferSubData(mInstanceBufferId, *id * sizeof(GPUInstanceData), sizeof(GPUInstanceData), &mInstances[*id]));
}

OID RendererDriverOpenGL::CreateCanvasItem() {
    //OID id = Base::CreateCanvasItem();
...
void RendererDriverOpenGL::DestroyCanvasItem(OID id) {
...
void RendererDriverOpenGL::SetCanvasItemData(OID id, const ArrayView<const Vertex2D>& vertices, const ArrayView<const int>& indices) {
...
void RendererDriverOpenGL::SetCanvasItemTexture(OID id, OID textureId) {
}

void RendererDriverOpenGL::DrawCanvasItem(OID id, int vertexOffset, int indexOffset, int indexCount) {
...
void RendererDriverOpenGL::LLSetTexture2D(OID textureId) {
    OID newTextureId = IsTextureValid(textureId) ? textureId : GetWhiteTexture();
